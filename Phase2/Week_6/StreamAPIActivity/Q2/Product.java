...
public class Product {
	...
	public List<Product> filterProducts(Double price){
        ...
        List<Product> newList = list.stream().filter(p -> p.getPrice()<=price).collect(Collectors.toList());
        return newList;
	}
    public String toString()
    {
        return String.format("%-20s %-20s %-20s %-10s %s", this.getName(), this.getBrand(), this.getProductCategory(), this.getColor(), this.getPrice());
    }
